...
class CosLoss(nn.Module):
    ...
    def forward(self, predicted,target):
        ...
        exp_s = np.e ** (self.s * torch.cos(theta_j) - self.m)
...
class Sphereface(nn.Module):
  ...
  # AMSoftmax
  def __init__(self, margin=4, s=30):
      super().__init__()
  ...
  def forward(self, predicted, target):
      ...
      # ------------ AM Softmax ------------ #
      #torch.Size([128, 462]) torch.Size([128])
      predicted = predicted / (predicted.norm(p=2, dim=0) + self.epsilon)

      indexes = range(predicted.size(0))
      cos_theta_y = predicted[indexes, target] # shape = 128
      cos_theta_y_m = cos_theta_y * self.m
      exp_s = np.e ** (cos_theta_y_m)

      sum_cos_theta_j = (np.e ** (predicted)).sum(dim=1) - (np.e ** (predicted[indexes, target]))
...
for epoch in range(N_epochs):
  ...
  for i in range(N_batches):
    ...
    loss = cost1(pout, lab.long()) + cost2(pout, lab.long()) + cost3(pout, lab.long())
